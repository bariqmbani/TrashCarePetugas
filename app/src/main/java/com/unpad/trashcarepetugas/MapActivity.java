...
//    private LokasiWarga mLokasiWarga;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        for (LokasiWarga lokasiWarga : mLokasiWarga) {
            Log.d(TAG, "onCreateView: lokasi warga: " + lokasiWarga.getWarga().getNama());
            Log.d(TAG, "onCreateView: geopoint: " + lokasiWarga.getGeo_point().getLatitude() + ", " + lokasiWarga.getGeo_point().getLongitude());
        }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mWargaListRecyclerView = view.findViewById(R.id.warga_list_recycler_view);
        mMapView = view.findViewById(R.id.warga_list_map);
    ...
    /*private void setCameraView() {
        ...
        double bottomBoundary = mPosisiPetugas.getGeo_point().getLatitude() - .1;
        double leftBoundary = mPosisiPetugas.getGeo_point().getLongitude() - .1;
        double topBoundary = mPosisiPetugas.getGeo_point().getLatitude() + .1;
        double rightBoundary = mPosisiPetugas.getGeo_point().getLongitude() + .1;
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    for (QueryDocumentSnapshot document : task.getResult()) {
                        ...
                        for (LokasiWarga lokasi : mLokasiWarga) {
                            Log.d(TAG, "onCreateView: lokasi warga: " + lokasi.getWarga().getNama());
                            Log.d(TAG, "onCreateView: geopoint: " + lokasi.getGeo_point().getLatitude() + ", " + lokasi.getGeo_point().getLongitude());
                        }
                    }
                    /*mWargaRecyclerAdapter = new WargaRecyclerAdapter(mWargaList);
                    RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(MapActivity.this);
                    mWargaListRecyclerView.setLayoutManager(layoutManager);
...
//                    Toast.makeText(this, "gagal", Toast.LENGTH_SHORT).show();
    ...
    private void getLokasiWarga(LokasiWarga lokasiWarga) {
        Task<DocumentSnapshot> lokasiRef = db.collection("Lokasi Warga").document().get();

        lokasiRef.addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
